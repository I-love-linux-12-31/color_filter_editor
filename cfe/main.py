...
def main():
    ...
    if not sys_info.get_gpus_names():
        ...
        gpus = [F"{hex(i['id'])} {i['name']}\n Capabilities: {', '.join(i['capabilities'])}" for i in pc_info["devices"]]
        # sys_info.get_gpus_names()
        if len(gpus) > 1:  # 2
            main_window.label_gpu.setMinimumSize(QtCore.QSize(0, 128))
        if len(gpus) > 2:  # > 2
            main_window.label_gpu.setMinimumSize(QtCore.QSize(0, 200))

        main_window.label_gpu.setText("GPUS (Supported with current Xorg configuration):\n" + "\n".join(gpus))
